...
export default function TimeFallingAway() {
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      if (viewMode === 'water' && timestamp - lastDropletTime.current > 150) {
        ...
        const drainX =
          tankBounds.left + (tankBounds.right - tankBounds.left) * 0.5;
  ...
  const renderWaterTankVisualization = (updatedStats: LifeStats) => {
    ...
    const percentage = isShowingToday
      ? todayStats.percentagePassedToday
      : updatedStats.percentageLived;
    const remaining = isShowingToday
      ? todayStats.secondsRemainingToday
      : updatedStats.secondsRemaining;
  ...
  const renderProgressBarVisualization = (updatedStats: LifeStats) => {
    return (
      <div className="bg-black/50 backdrop-blur-sm border border-gray-700 rounded-lg p-4 sm:p-6 lg:p-8 mb-6 lg:mb-12">
        <h2 className="text-lg sm:text-xl lg:text-2xl font-semibold text-white mb-4 lg:mb-6">
          Life Progress
        </h2>
        <div className="relative">
          ...
          <div className="flex justify-between mt-2 lg:mt-4 text-sm lg:text-base">
            <span className="text-red-400">
              {updatedStats.percentageLived}% lived
            </span>
            <span className="text-gray-400">
              {100 - updatedStats.percentageLived}% remaining
            </span>
          </div>
        </div>
      </div>
    ...
    return (
      <div className="relative min-h-screen">
        <canvas
          ...
          style={{
            background:
              'linear-gradient(180deg, rgba(17,24,39,0.95) 0%, rgba(31,41,55,0.98) 100%)',
          }}
        />
        ...
        <div className="relative z-10 p-4 sm:p-6 lg:p-8 pt-16">
          <div className="max-w-4xl lg:max-w-6xl xl:max-w-7xl mx-auto">
            <div className="text-center mb-8 lg:mb-16">
              ...
              <p className="text-lg sm:text-xl lg:text-2xl text-red-400 mb-2 lg:mb-4">
                {t('timeRemainingMessage')}
              </p>
            ...
            <div className="grid md:grid-cols-2 xl:grid-cols-2 gap-4 sm:gap-6 lg:gap-8 mb-6 lg:mb-12">
              <div className="bg-black/50 backdrop-blur-sm border border-gray-700 rounded-lg p-4 sm:p-6 lg:p-8">
                ...
                <div className="space-y-2 lg:space-y-4 text-gray-300">
                  <p>
                    <span className="text-red-400 font-mono text-base sm:text-lg lg:text-xl">
                      {getFormattedNumber(updatedStats.secondsLived)}
                    </span>{' '}
                    seconds lived
                  </p>
                  <p>
                    <span className="text-red-400 font-mono text-base sm:text-lg lg:text-xl">
                      {getFormattedNumber(updatedStats.heartbeats)}
                    </span>{' '}
                    heartbeats
                  </p>
                  <p>
                    <span className="text-red-400 font-mono text-base sm:text-lg lg:text-xl">
                      {getFormattedNumber(updatedStats.breaths)}
                    </span>{' '}
                    breaths taken
                  </p>
                  <p>
                    <span className="text-red-400 font-mono text-base sm:text-lg lg:text-xl">
                      {getFormattedNumber(updatedStats.weekendsLived)}
                    </span>{' '}
              ...
              <div className="bg-black/50 backdrop-blur-sm border border-blue-700 rounded-lg p-4 sm:p-6 lg:p-8">
                ...
                <div className="space-y-2 lg:space-y-4 text-gray-300">
                  <p>
                    <span className="text-blue-400 font-mono text-base sm:text-lg lg:text-xl">
                      {getFormattedNumber(updatedStats.secondsRemaining)}
                    </span>{' '}
                    seconds left
                  </p>
                  <p>
                    <span className="text-blue-400 font-mono text-base sm:text-lg lg:text-xl">
                      {getFormattedNumber(
                        Math.floor((updatedStats.secondsRemaining * 70) / 60)
                      )}
                    </span>{' '}
                  ...
                  <p>
                    <span className="text-blue-400 font-mono text-base sm:text-lg lg:text-xl">
                      {getFormattedNumber(
                        Math.floor((updatedStats.secondsRemaining * 16) / 60)
                      )}
                    </span>{' '}
                    breaths to take
                  </p>
                  <p>
                    <span className="text-blue-400 font-mono text-base sm:text-lg lg:text-xl">
                      {getFormattedNumber(updatedStats.weekendsRemaining)}
                    </span>{' '}
                    weekends ahead
                  </p>
                </div>
            ...
            <div className="bg-black/50 backdrop-blur-sm border border-gray-700 rounded-lg p-4 sm:p-6 lg:p-8 mb-6 lg:mb-12">
              <h3 className="text-base sm:text-lg lg:text-xl font-semibold text-white mb-3 lg:mb-6">
                What You&apos;re Seeing
              </h3>
              <div className="grid sm:grid-cols-3 gap-3 lg:gap-6 text-sm lg:text-base">
                <div className="flex items-center">
                  ...
                  <span className="text-gray-300">
                    Your past seconds falling away
                  </span>
                </div>
                <div className="flex items-center">
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black flex items-center justify-center p-4 sm:p-6 lg:p-8">
        <div className="max-w-sm sm:max-w-md lg:max-w-lg xl:max-w-xl mx-auto">
          <div className="text-center mb-6 lg:mb-12">
            <h1 className="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-bold text-white mb-3 lg:mb-6">
              {t('pageTitle')}
            </h1>
          ...
          <div className="bg-black/50 backdrop-blur-sm border border-gray-700 rounded-lg p-4 sm:p-6 lg:p-8">
            <h2 className="text-base sm:text-lg lg:text-xl font-semibold mb-3 lg:mb-6 text-white">
              {t('birthDateQuestion')}
            </h2>
            <input
              type="date"
