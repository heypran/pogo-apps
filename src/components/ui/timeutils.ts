...
export const TRANSLATIONS = {
  'en-US': {
    ...
    pageSubtitle:
      'Watch your precious moments drift by... each second that passes is gone forever',
    ...
    urgencyMessage:
      'Every second counts. What will you do with the time you have left?',
  },
};
...
export const calculateTodayStats = (): TodayStats => {
  ...
  const secondsPassedToday = Math.floor(
    (now.getTime() - startOfDay.getTime()) / 1000
  );
  const secondsRemainingToday = totalSecondsInDay - secondsPassedToday;
  const percentagePassedToday = Math.round(
    (secondsPassedToday / totalSecondsInDay) * 100
  );
...
export const calculateStats = (date: string, currentTime: Date): LifeStats => {
  ...
  const percentageLived = Math.min(
    100,
    Math.round((secondsLived / totalSeconds) * 100)
  );
